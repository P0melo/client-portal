        ...
        where TUser :IdentityUser
    {
        ...
        public TUser GetUserById(string userId)
        {
            ...
            if (rows != null && rows.Count == 1)
            {
                ...
                user.CompanyId = string.IsNullOrEmpty(row["CompanyId"]) ? 0 : int.Parse(row["CompanyId"]);
                user.CompanyName = string.IsNullOrEmpty(row["CompanyName"]) ? null : row["CompanyName"];
            }
        ...
        {
            string commandText = @"Insert into Users (UserName, Id, PasswordHash, SecurityStamp,Email,EmailConfirmed,PhoneNumber,PhoneNumberConfirmed, AccessFailedCount,LockoutEnabled,LockoutEndDateUtc,TwoFactorEnabled, FirstName, LastName, CompanyId, CompanyName)
                values (@name, @id, @pwdHash, @SecStamp,@email,@emailconfirmed,@phonenumber,@phonenumberconfirmed,@accesscount,@lockoutenabled,@lockoutenddate,@twofactorenabled, @firstname, @lastname, @companyid, @company)";
            Dictionary<string, object> parameters = new Dictionary<string, object>();
            ...
            parameters.Add("@company", user.CompanyName);
        ...
        public int Update(TUser user)
        {
            string commandText = @"Update Users set UserName = @userName, PasswordHash = @pswHash, SecurityStamp = @secStamp, 
                ...
                FirstName=@firstname, LastName=@lastname, CompanyId=@companyid, CompanyName=@company
                WHERE Id = @userId";
            Dictionary<string, object> parameters = new Dictionary<string, object>();
